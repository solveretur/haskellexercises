...
lowerAndGreater n list = do
  let g = length [x | x <- list, n > x]
  let l = length [x | x <- list, n < x]
  show n ++ " is greater than " ++ show g ++ " elements and lower than " ++ show l ++" elements"
